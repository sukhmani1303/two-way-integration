...
@router.post('/outward_sync/delete', status_code = status.HTTP_200_OK, tags = ['outward_sync'])
async def all(fdata : schemas.lookup, db : Session = Depends(get_db)):
    q1 = outward.delete(db, fdata)
    print(q1)
    if q1 is not None:
        item_data = dict(fdata)
        k_producer.add_item_to_queue(item_data, "delete")
        return JSONResponse(content={"message": "Customer Catalog Updated Successfully!"}, status_code=200)
...
@router.post("/inward_sync/delete",status_code = status.HTTP_200_OK ,tags = ['inward_sync'])
async def stripe_webhook(event: dict, db : Session = Depends(get_db)):
        try:
            if event['type'] == 'customer.deleted':
                ...
                resp = inward.delete_customer_to_local_catalog(customer, db)
